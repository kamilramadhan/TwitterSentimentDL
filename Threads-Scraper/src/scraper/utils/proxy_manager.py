    ...
    def _load(self) -> List[Dict[str, str]]:
        ...
        try:
            with open(self.proxies_path, "r", encoding="utf-8") as f:
                raw = f.read().strip()
            if not raw:
                logger.info(f"Proxy file is empty at {self.proxies_path}. Proceeding without proxies.")
                return []
            data = json.loads(raw)
            ...
            logger.warning(f"Failed to parse proxy file at {self.proxies_path}: {e}. Proceeding without proxies.")
            return []
